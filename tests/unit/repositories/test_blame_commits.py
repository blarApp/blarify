...
class TestBlameCommitDiscovery:
    ...
    def test_blame_commits_for_nodes(self):
        ...
        def mock_blame_response(
            file_path: str, start_line: int, end_line: int, ref: str = "HEAD"
        ) -> List[Dict[str, Any]]:
    ...
    def test_ranges_overlap_detection(self):
        ...
        assert github._ranges_overlap([{"start": 10, "end": 20}], 15, 25) is True

        # Test non-overlapping ranges
        assert github._ranges_overlap([{"start": 10, "end": 20}], 25, 30) is False

        # Test exact match
        assert github._ranges_overlap([{"start": 10, "end": 20}], 10, 20) is True

        # Test contained range
        assert github._ranges_overlap([{"start": 10, "end": 30}], 15, 25) is True
