...
class GraphAssertions:
    ...
    async def assert_node_count(
        ...
        assert actual_count == expected_count, (
            f"Expected {expected_count} nodes with label '{label}', "
    ...
    async def assert_nodes_connected_by_path(
        ...
        assert count > 0, (
            f"No path found matching pattern: ({start_label}){path_pattern}({end_label})"
        )
    ...
    async def assert_graph_structure_basics(
        ...
        assert not missing_relationships, (
            f"Missing expected relationship types: {missing_relationships}"
        )
