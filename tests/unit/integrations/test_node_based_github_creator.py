...
class TestNodeBasedGitHubCreator:
    ...
    def test_process_existing_nodes(self, github_creator, mock_db_manager):
        ...
        # Create proper DTO objects for blame results
        pr_info = PullRequestInfoDto(
            number=123, title="Fix auth", url="https://github.com/owner/repo/pull/123", author="alice", merged_at=None
        )

        line_ranges1 = [BlameLineRangeDto(start=15, end=25)]
        commit1 = BlameCommitDto(
            sha="abc123",
            ...
            line_ranges=line_ranges1,
            pr_info=pr_info,
        )

        line_ranges2 = [BlameLineRangeDto(start=5, end=15)]
        commit2 = BlameCommitDto(
            sha="def456",
        ...
        with patch.object(github_creator.github_repo, "blame_commits_for_nodes", return_value=blame_results):
            ...
            result = github_creator.create_github_integration_from_nodes(node_ids=["node1", "node2"])
    ...
    def test_create_integration_from_blame(self, github_creator):
        ...
        line_ranges = [BlameLineRangeDto(start=10, end=20)]
        commit1 = BlameCommitDto(
            sha="abc123",
        ...
        line_ranges2 = [BlameLineRangeDto(start=1, end=10)]
        commit2 = BlameCommitDto(
            sha="def456",
            message="Add feature",
            author="Bob",
    ...
    def test_query_all_code_nodes(self, github_creator, mock_db_manager):
        ...
        # Verify returned nodes are DTOs with correct data
        assert len(nodes) == 2
        assert isinstance(nodes[0], CodeNodeDto)
        assert nodes[0].id == "n1"
        assert nodes[0].name == "main"
        assert nodes[0].label == "FUNCTION"
    ...
    def test_modified_by_with_blame_relationship(self, github_creator):
        ...
        with patch.object(RelationshipCreator, "create_modified_by_with_blame") as mock_create:
            ...
            rel = RelationshipCreator.create_modified_by_with_blame(
                commit_node=commit_node, code_node=code_node, line_ranges=line_ranges
            )

            mock_create.assert_called_once_with(commit_node=commit_node, code_node=code_node, line_ranges=line_ranges)
    ...
    def test_save_to_database_flag(self, github_creator, mock_db_manager):
        ...
        with patch.object(github_creator.github_repo, "blame_commits_for_nodes", return_value=blame_results):
            ...
            github_creator.create_github_integration_from_nodes(node_ids=["node1"], save_to_database=False)
            ...
            github_creator.create_github_integration_from_nodes(node_ids=["node1"], save_to_database=True)
    ...
    def test_error_handling(self, github_creator):
        ...
        # Mock database query
        github_creator.db_manager.query.return_value = [
            {
                "id": "node1",
                "name": "test_func",
                "label": "FUNCTION",
        ...
        with patch.object(github_creator.github_repo, "blame_commits_for_nodes", side_effect=Exception("API Error")):
            result = github_creator.create_github_integration_from_nodes(node_ids=["node1"])
