    ...
    async def test_belongs_to_workflow_relationships_created(
        ...
        for expected in expected_participants:
            assert expected in participant_names, (
                f"Function '{expected}' not connected to workflow. "
                f"Found participants: {participant_names}"
            )
        ...
        """
        duplicates = await neo4j_instance.execute_cypher(
            duplicate_query,
            {"workflow_id": workflow_id}
        )
            ...
            if mutual_results:
                for result in mutual_results:
                    assert set(result["functions"]) == {"is_even", "is_odd"}, (
                        f"Mutual recursion workflow missing expected functions. "
                        f"Found: {result['functions']}"
                    )
        else:
...
""")
        ...
        for workflow in workflows:
            if workflow["steps"] == 1:  # Single node workflow
                ...
                """
                results = await neo4j_instance.execute_cypher(
                    relationship_query,
    ...
    async def test_workflow_relationships_with_targeted_node_path(
        ...
        for workflow in workflows:
            ...
            assert "transform_value" in participant_names, (
                f"Target function 'transform_value' not in workflow participants. "
...
class WorkflowGraphAssertions(GraphAssertions):
    ...
    async def assert_belongs_to_workflow_exists(
        self, 
        node_name: str, 
        workflow_id: str
    ) -> None:
        ...
        """
        result = await self.neo4j_instance.execute_cypher(
            query,
            {"node_name": node_name, "workflow_id": workflow_id}
        )
        
        assert result[0]["count"] > 0, (
            f"No BELONGS_TO_WORKFLOW relationship found between node '{node_name}' "
            f"and workflow '{workflow_id}'"
        )
    ...
    async def assert_workflow_complete(self, workflow_id: str) -> Dict[str, Any]:
        ...
        """
        step_result = await self.neo4j_instance.execute_cypher(
            step_query,
            {"workflow_id": workflow_id}
        )
        ...
        """
        belongs_result = await self.neo4j_instance.execute_cypher(
            belongs_query,
            {"workflow_id": workflow_id}
        )
