...
class CodebaseSkeletonInput(BaseModel):
    ...
    entity_id: str = Field(
        description="The entity ID (company_id) to retrieve the codebase skeleton for"
    )
    environment: str = Field(
        default="default",
        description="The environment to query (default: 'default')"
    )
...
class GetRootCodebaseSkeletonTool(BaseTool):
    ...
    db_manager: AbstractDbManager = Field(
        description="Database manager instance for querying the graph database"
    )
    ...
    async def _arun(
        ...
        return self._run(entity_id, environment, run_manager)