...
class Neo4jTestContainerManager:
    """
    Manages Neo4j test container lifecycle.
    ...
    async def _create_and_start_container(self, config: Neo4jContainerConfig,
                                        ports: PortAllocation, volume: VolumeInfo) -> Neo4jContainerInstance:
        ...
        try:
            ...
            neo4j_container = (
                neo4j_container
                .with_name(config.container_name)
    ...
    async def _wait_for_neo4j_ready(self, instance: Neo4jContainerInstance, timeout: int) -> None:
        ...
        raise ContainerStartupError(
            f"Neo4j container did not become ready within {timeout} seconds"
        )
    ...
    async def wait_for_container_healthy(self, container_id: str, 
    ...
    def __enter__(self) -> "Neo4jTestContainerManager":
    ...
    async def __aenter__(self) -> "Neo4jTestContainerManager":
