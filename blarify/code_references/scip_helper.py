...
class ScipReferenceResolver:
    ...
    def ensure_loaded(self) -> bool:
        ...
        try:
            ...
            logger.info(f"📚 Loaded SCIP index in {load_time:.2f}s: {len(self._document_by_path)} documents, {len(self._symbol_to_occurrences)} symbols")
            self._loaded = True
            return True
    ...
    def get_references_for_node(self, node: DefinitionNode) -> List[Reference]:
        ...
        for occurrence in occurrences:
            ...
            if not (occurrence.symbol_roles & (scip.SymbolRole.ReadAccess | scip.SymbolRole.WriteAccess | scip.SymbolRole.Import)):
                continue
    ...
    def get_references_batch_with_progress(self, nodes: List[DefinitionNode]) -> Dict[DefinitionNode, List[Reference]]:
        ...
        logger.info(f"📊 Found symbols for {len(nodes_with_symbols)}/{total_nodes} nodes ({len(nodes_with_symbols)/total_nodes*100:.1f}%)")
    ...
    def _find_symbol_for_node(self, node: DefinitionNode) -> Optional[str]:
        ...
        relative_path = PathCalculator.get_relative_path_from_uri(
            root_uri=f"file://{self.root_path}", 
            uri=node.path
        )
        ...
        for occurrence in document.occurrences:
            ...
            if (occurrence.range and len(occurrence.range) >= 2 and
                occurrence.range[0] == node.definition_range.start_dict["line"] and
                occurrence.range[1] == node.definition_range.start_dict["character"]):
