...
class ProgressTracker:
    ...
    def _log_progress(self):
        ...
        logger.info(f"🔄 Progress: [{bar}] {self.completed_nodes}/{self.total_nodes} ({percentage:.1f}%) | {elapsed_time:.0f}s elapsed | {eta_str}")
    ...
    def complete(self):
        ...
        with self.lock:
            ...
            bar = '█' * 30
            logger.info(f"✅ Complete: [{bar}] {self.total_nodes}/{self.total_nodes} (100.0%) | Total time: {elapsed_time:.1f}s")
...
class LspResourceOptimizer:
    ...
    def get_optimal_lsp_instances(language: str = None) -> int:
        ...
        language_factors = {
            ...
            "python": {"memory_per_instance_mb": 300, "cpu_efficiency": 0.8, "max_recommended": 8},
            "typescript": {"memory_per_instance_mb": 400, "cpu_efficiency": 0.7, "max_recommended": 6},
            "javascript": {"memory_per_instance_mb": 350, "cpu_efficiency": 0.8, "max_recommended": 8},
            "java": {"memory_per_instance_mb": 500, "cpu_efficiency": 0.6, "max_recommended": 4},
            "csharp": {"memory_per_instance_mb": 450, "cpu_efficiency": 0.6, "max_recommended": 4},
            
            # Lighter languages
            "go": {"memory_per_instance_mb": 150, "cpu_efficiency": 0.9, "max_recommended": 12},
            "rust": {"memory_per_instance_mb": 200, "cpu_efficiency": 0.9, "max_recommended": 10},
            "ruby": {"memory_per_instance_mb": 250, "cpu_efficiency": 0.8, "max_recommended": 8},
            "php": {"memory_per_instance_mb": 200, "cpu_efficiency": 0.8, "max_recommended": 8},
        }
        
        # Default values for unknown languages
        default_factor = {"memory_per_instance_mb": 300, "cpu_efficiency": 0.8, "max_recommended": 6}
        ...
        logger.info(f"🔧 LSP optimization for {language or 'unknown'}: {cpu_cores} cores, {memory_gb:.1f}GB RAM → {optimal} instances")
        logger.debug(f"   CPU-based: {cpu_based}, Memory-based: {memory_based}, Language max: {factor['max_recommended']}")
...
class LspQueryHelper:
    ...
    def __init__(self, root_uri: str, host: Optional[str] = None, port: Optional[int] = None, max_lsp_instances: int = None, base_timeout: int = 30, per_request_timeout: int = 2, max_batch_size: int = 50, auto_optimize: bool = True):
        ...
        if auto_optimize:
            ...
            logger.info(f"💻 System resources: {system_info['cpu_cores']} cores, {system_info['memory_gb']:.1f}GB RAM ({system_info['available_memory_gb']:.1f}GB available)")
        else:
    ...
    def get_paths_where_nodes_are_referenced_batch(self, nodes: List["DefinitionNode"]) -> Dict["DefinitionNode", List[Reference]]:
        """
        ...
        for lang_index, (language, language_nodes) in enumerate(nodes_by_language.items(), 1):
            try:
                ...
                logger.info(f"🔧 [{lang_index}/{len(nodes_by_language)}] Processing {len(language_nodes)} {language} nodes with {len(lsp_servers)} LSP server instances")
    ...
    def _batch_request_references_with_multiple_servers(self, nodes: List["DefinitionNode"], lsp_servers: List[SyncLanguageServer], progress: ProgressTracker) -> Dict["DefinitionNode", List[Reference]]:
        """
    ...
    def _batch_request_references_for_language(self, nodes: List["DefinitionNode"], lsp_server: SyncLanguageServer, progress: ProgressTracker = None) -> Dict["DefinitionNode", List[Reference]]:
        """
        ...
        try:
            ...
            logger.error(f"⏰ Batch request timed out after {dynamic_timeout}s for {len(nodes)} nodes, falling back to smaller batches")
            # Fallback: split into smaller chunks and retry
            return self._process_large_batch_in_chunks(nodes, lsp_server, progress, chunk_size=max(1, self.MAX_BATCH_SIZE // 2))
        except Exception as e:
    ...
    def _process_large_batch_in_chunks(self, nodes: List["DefinitionNode"], lsp_server: SyncLanguageServer, progress: ProgressTracker = None, chunk_size: int = None) -> Dict["DefinitionNode", List[Reference]]:
    ...
    def _batch_request_references_simple(self, nodes: List["DefinitionNode"], lsp_server: SyncLanguageServer, progress: ProgressTracker = None) -> Dict["DefinitionNode", List[Reference]]:
        """
    ...
    def shutdown_exit_close(self) -> None:
        ...
        for language in languages:
            try:
                ...
                logger.error(f"Error shutting down LSP servers for {language}: {e}")

        # Ensure all dictionaries are cleared
        self.entered_lsp_servers.clear()
        self.language_to_lsp_servers.clear()
        logger.info("All LSP server instances have been shut down")